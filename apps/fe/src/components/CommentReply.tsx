...
export default function CommentReply({ reply, currentUserId, onAddReply, canComment }: CommentReplyProps) {
  ...
  const handleSubmitReply = () => {
    ...
    onAddReply(reply.parentId ?? reply.id, replyText.trim());
    setReplyText("");
      ...
      <div className="flex-1 min-w-0 pb-2">
        <div className="flex items-center justify-between mb-0.5">
          ...
          <div className="flex items-center gap-1.5 text-[#777]">
            <span className="text-xs">{formatTime(reply.createdAt)}</span>
            <DropdownMenu>
        ...
        <p className="text-[14px] leading-5 text-[#F3F5F7] mb-2">{reply.content}</p>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <Heart size={14} className={liked ? "fill-[#FF2E40] text-[#FF2E40]" : ""} />
        ...
        {showReplyForm && (
          <div className="mt-2 flex items-start gap-2">
            <textarea
              ...
              rows={2}
              className="flex-1 bg-[#1E1E1E] text-[13px] text-[#F3F5F7] placeholder:text-[#777] resize-none outline-none rounded-xl px-3 py-2 border border-[#3E4042] focus:border-[#777] transition-colors"
            />
